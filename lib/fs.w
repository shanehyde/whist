...
private extern fs {
    ...
    // Handle-based API
    func fs__open(path: string, mode: string): i64;
    func fs__close(handle: i64): i32;
    func fs__read_line(handle: i64): string;
    func fs__write_string(handle: i64, content: string): i32;
    func fs__flush(handle: i64): i32;
    func fs__seek(handle: i64, offset: i64, whence: i32): i32;
    func fs__tell(handle: i64): i64;
    func fs__eof(handle: i64): bool;
}
...
func open(path: string, mode: string): i64 {
    return fs__open(path, mode);
}

func close(handle: i64): i32 {
    return fs__close(handle);
}

func read_line(handle: i64): string {
    return fs__read_line(handle);
}

func write_string(handle: i64, content: string): i32 {
    return fs__write_string(handle, content);
}

func flush(handle: i64): i32 {
    return fs__flush(handle);
}

func seek(handle: i64, offset: i64, whence: i32): i32 {
    return fs__seek(handle, offset, whence);
}

func tell(handle: i64): i64 {
    return fs__tell(handle);
}

func eof(handle: i64): bool {
