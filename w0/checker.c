...
static Type *resolve_type(Checker *checker, Node *type_node) {
    ...
    switch (type_node->type) {
        case NODE_IDENT: {
            ...
            error(checker, type_node->line, type_node->column,
                  "Unknown type '%s'", name);
            return type_error;
        }
        case NODE_UNARY:
...
static Type *check_expr(Checker *checker, Node *node) {
    ...
    switch (node->type) {
        ...
        case NODE_IDENT: {
            ...
            if (!sym) {
                error(checker, node->line, node->column,
                      "Undefined identifier '%s'", node->as.ident.name);
                return type_error;
            }
            return sym->type;
        }
        ...
        case NODE_UNARY: {
            ...
            if (operand->kind == TYPE_ERROR) return type_error;
        ...
        case NODE_CALL: {
            ...
            if (func_type->kind == TYPE_ERROR) return type_error;

            if (func_type->kind != TYPE_FUNC) {
                error(checker, node->line, node->column,
                      "Cannot call non-function type '%s'", type_name(func_type));
                return type_error;
            }
        ...
        case NODE_INDEX: {
            ...
            if (index->kind != TYPE_INT) {
                error(checker, node->line, node->column,
                      "Array index must be int, got '%s'", type_name(index));
                return type_error;
            }
            ...
            error(checker, node->line, node->column,
                  "Cannot index type '%s'", type_name(object));
            return type_error;
        }
        ...
        case NODE_MEMBER: {
            ...
            if (object->kind == TYPE_ERROR) return type_error;
        ...
        case NODE_ASSIGN: {
            ...
            if (t->type == NODE_IDENT) {
                ...
                if (sym && sym->is_const) {
                    error(checker, node->line, node->column,
                          "Cannot assign to const '%s'", t->as.ident.name);
                    return type_error;
                }
            }
    ...
    switch (node->type) {
            ...
            if (decl_type && init_type) {
                ...
                if (!type_assignable(decl_type, init_type)) {
                    error(checker, node->line, node->column,
                          ...
                          name, type_name(decl_type), type_name(init_type));
                }
                var_type = decl_type;
            } else if (decl_type) {
                var_type = decl_type;
        ...
        case NODE_IF: {
            ...
            if (cond->kind != TYPE_BOOL && cond->kind != TYPE_ERROR) {
                error(checker, node->as.if_stmt.cond->line,
                      node->as.if_stmt.cond->column,
        ...
        case NODE_WHILE: {
            ...
            if (cond->kind != TYPE_BOOL && cond->kind != TYPE_ERROR) {
                error(checker, node->as.while_stmt.cond->line,
                      node->as.while_stmt.cond->column,
        ...
        case NODE_FOR: {
            ...
            if (node->as.for_stmt.cond) {
                ...
                if (cond->kind != TYPE_BOOL && cond->kind != TYPE_ERROR) {
                    error(checker, node->as.for_stmt.cond->line,
                          node->as.for_stmt.cond->column,
    ...
    switch (node->type) {
        case NODE_FUNC_DECL: {
            ...
            for (int i = 0; i < param_count; i++) {
                ...
                if (!checker_define(checker, param->as.param.name, SYM_VAR, ptype)) {
                    error(checker, param->line, param->column,
                          "Duplicate parameter name '%s'", param->as.param.name);
                }
            }
        ...
        case NODE_ENUM_DECL: {
            ...
            for (int i = 0; i < value_count; i++) {
                ...
                if (!sym) {
                    error(checker, val->line, val->column,
                          "Redefinition of '%s'", val->as.ident.name);
                }
            }
            break;
        }
        ...
        default:
            error(checker, node->line, node->column,
                  "Unknown declaration type %d", node->type);
            break;
    }
}
