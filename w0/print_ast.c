...
// --- Checked AST printer (shows checker-set flags) ---

static void print_checked_node_list(const char* label, NodeList* list, int depth) {
    if (label) {
        print_indent(depth);
        printf("%s:\n", label);
    }
    for (int i = 0; i < list->count; i++) {
        print_ast_checked(list->nodes[i], depth + 1);
    }
}

static void print_checked_labeled_child(const char* label, Node* child, int depth) {
    print_indent(depth);
    printf("%s:\n", label);
    print_ast_checked(child, depth + 1);
}

static void print_checked_optional(const char* label, Node* child, int depth) {
    if (child) {
        print_checked_labeled_child(label, child, depth);
    }
}

// Print checker annotations in [brackets] for a node
static void print_checker_flags(Node* node) {
    int first = 1;
    ...
    case NODE_VAR_DECL:
        ...
        if (node->as.var_decl.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ", type_name(node->as.var_decl.resolved_type));
            first = 0;
        }
        break;
    case NODE_BINARY:
        if (node->as.binary.is_string_op) {
            printf("%sstring_op", first ? " [" : ", ");
            first = 0;
        }
        ...
        if (node->as.binary.is_enum_eq) {
            printf("%senum_eq", first ? " [" : ", ");
            first = 0;
        }
        if (node->as.binary.eq_type_name) {
            printf("%seq_type=%s", first ? " [" : ", ", node->as.binary.eq_type_name);
            first = 0;
        }
        break;
    case NODE_MEMBER:
        if (node->as.member.struct_name) {
            printf("%sstruct=%s", first ? " [" : ", ", node->as.member.struct_name);
            first = 0;
        }
        if (node->as.member.module_name) {
            printf("%smodule=%s", first ? " [" : ", ", node->as.member.module_name);
            first = 0;
        }
        if (node->as.member.is_ref) {
            printf("%sref", first ? " [" : ", ");
            first = 0;
        }
        if (node->as.member.is_method_ref) {
            printf("%smethod_ref", first ? " [" : ", ");
            first = 0;
        }
        break;
    case NODE_CALL:
        if (node->as.call.resolved_name) {
            printf("%sresolved=%s", first ? " [" : ", ", node->as.call.resolved_name);
            first = 0;
        }
        if (node->as.call.is_indirect_call) {
            printf("%sindirect", first ? " [" : ", ");
            first = 0;
        }
        break;
    case NODE_INDEX:
        if (node->as.index.is_tuple_index) {
            printf("%stuple_index", first ? " [" : ", ");
            first = 0;
        }
        if (node->as.index.is_span_index) {
            printf("%sspan_index", first ? " [" : ", ");
            first = 0;
        }
        if (node->as.index.is_vec_index) {
            printf("%svec_index", first ? " [" : ", ");
            first = 0;
        }
        if (node->as.index.is_rc_elem) {
            printf("%src_elem", first ? " [" : ", ");
            first = 0;
        }
        break;
    case NODE_SLICE:
        if (node->as.slice.is_string) {
            printf("%sstring", first ? " [" : ", ");
            first = 0;
        }
        ...
        if (node->as.slice.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ", type_name(node->as.slice.resolved_type));
            first = 0;
        }
        break;
    case NODE_NEW_EXPR:
        if (node->as.new_expr.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ", type_name(node->as.new_expr.resolved_type));
            first = 0;
        }
        break;
    case NODE_ENUM_VALUE:
        if (node->as.enum_value.is_data_enum) {
            printf("%sdata_enum", first ? " [" : ", ");
            first = 0;
        }
        break;
    case NODE_LAMBDA:
        if (node->as.lambda.lambda_id > 0) {
            printf("%slambda_id=%d", first ? " [" : ", ", node->as.lambda.lambda_id);
            first = 0;
        }
        if (node->as.lambda.captures.count > 0) {
            printf("%scaptures=%d", first ? " [" : ", ", node->as.lambda.captures.count);
            first = 0;
        }
        if (node->as.lambda.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ", type_name(node->as.lambda.resolved_type));
            first = 0;
        }
        break;
    case NODE_FOREACH:
        if (node->as.foreach_stmt.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ",
                   type_name(node->as.foreach_stmt.resolved_type));
            first = 0;
        }
        if (node->as.foreach_stmt.is_span) {
            printf("%sspan", first ? " [" : ", ");
            first = 0;
        }
        ...
        break;
    case NODE_MATCH:
        if (node->as.match_stmt.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ", type_name(node->as.match_stmt.resolved_type));
            first = 0;
        }
        if (node->as.match_stmt.is_value_match) {
            printf("%svalue_match", first ? " [" : ", ");
            first = 0;
        }
        break;
    case NODE_TRY_EXPR:
        if (node->as.try_expr.is_option) {
            printf("%soption", first ? " [" : ", ");
            first = 0;
        }
        if (node->as.try_expr.enum_name) {
            printf("%senum=%s", first ? " [" : ", ", node->as.try_expr.enum_name);
            first = 0;
        }
        if (node->as.try_expr.unwrapped_type) {
            printf("%sunwrapped=%s", first ? " [" : ", ",
                   type_name(node->as.try_expr.unwrapped_type));
            first = 0;
        }
        break;
    case NODE_CAST:
        if (node->as.cast_expr.resolved_type) {
            printf("%stype=%s", first ? " [" : ", ", type_name(node->as.cast_expr.resolved_type));
            first = 0;
        }
        break;
    case NODE_IDENT:
        if (node->as.ident.resolved_func_type) {
            printf("%sfunc_type", first ? " [" : ", ");
            first = 0;
        }
        break;
    default:
    ...
    // Common flags on any node
    if (node->is_owned_temp) {
        printf("%sowned_temp", first ? " [" : ", ");
        first = 0;
        if (node->owned_temp_type) {
            printf("(%s)", type_name(node->owned_temp_type));
        }
    }
...
// Checked AST printer: same structure as print_ast but with checker flag annotations
void print_ast_checked(Node* node, int depth) {
    ...
    case NODE_PROGRAM:
        printf("Program\n");
        print_checked_node_list(NULL, &node->as.program.modules, depth);
        break;
    case NODE_MODULE:
        printf("Module: %.*s\n", node->as.module.name_length, node->as.module.name);
        print_checked_node_list(NULL, &node->as.module.decls, depth);
        break;
    case NODE_EXTERN_MODULE:
        printf("ExternModule: %.*s\n", node->as.extern_module.module_name_length,
               node->as.extern_module.module_name);
        print_checked_node_list(NULL, &node->as.extern_module.decls, depth);
        break;
    case NODE_FUNC_DECL:
        if (node->as.func_decl.receiver_type) {
            printf("MethodDecl: (%s%.*s) %.*s",
            ...
            printf("FuncDecl: %.*s (extern: %.*s)", node->as.func_decl.name_length,
                   node->as.func_decl.name, node->as.func_decl.extern_name_length,
                   node->as.func_decl.extern_name);
        } else {
            printf("FuncDecl: %.*s", node->as.func_decl.name_length, node->as.func_decl.name);
        }
        print_checker_flags(node);
        printf("\n");
        print_visibility(depth + 1, node->as.func_decl.is_public);
        if (node->as.func_decl.receiver_type_args.count > 0) {
            print_checked_node_list("ReceiverTypeArgs", &node->as.func_decl.receiver_type_args,
                                    depth + 1);
        }
        if (node->as.func_decl.params.count > 0) {
            print_checked_node_list("Params", &node->as.func_decl.params, depth + 1);
        }
        print_checked_optional("ReturnType", node->as.func_decl.return_type, depth + 1);
        print_checked_labeled_child("Body", node->as.func_decl.body, depth + 1);
        break;
    case NODE_PARAM:
        printf("Param: %.*s\n", node->as.param.name_length, node->as.param.name);
        print_checked_optional("Type", node->as.param.type, depth + 1);
        break;
    case NODE_STRUCT_DECL:
        printf("StructDecl: %.*s\n", node->as.struct_decl.name_length, node->as.struct_decl.name);
        print_visibility(depth + 1, node->as.struct_decl.is_public);
        ...
        print_checked_node_list(NULL, &node->as.struct_decl.fields, depth);
        break;
    case NODE_FIELD:
        printf("Field: %.*s\n", node->as.field.name_length, node->as.field.name);
        print_ast_checked(node->as.field.type, depth + 1);
        break;
    case NODE_ENUM_DECL:
        printf("EnumDecl: %.*s\n", node->as.enum_decl.name_length, node->as.enum_decl.name);
        print_visibility(depth + 1, node->as.enum_decl.is_public);
        print_checked_node_list(NULL, &node->as.enum_decl.values, depth);
        break;
    case NODE_ENUM_VARIANT: {
        printf("EnumVariant: %.*s", node->as.enum_variant.name_length, node->as.enum_variant.name);
        if (node->as.enum_variant.has_explicit_value) {
        ...
        printf("\n");
        print_checked_node_list(NULL, &node->as.enum_variant.types, depth);
        break;
    }
    case NODE_VAR_DECL:
        if (node->as.var_decl.destruct_pattern) {
            printf("VarDecl: ");
            print_destruct_pattern(node->as.var_decl.destruct_pattern);
            printf("%s", node->as.var_decl.is_const ? " (const)" : "");
        } else {
            printf("VarDecl: %.*s%s", node->as.var_decl.name_length, node->as.var_decl.name,
                   node->as.var_decl.is_const ? " (const)" : "");
        }
        print_checker_flags(node);
        printf("\n");
        if (node->as.var_decl.type) {
            print_checked_labeled_child("Type", node->as.var_decl.type, depth + 1);
        }
        if (node->as.var_decl.init) {
            print_checked_labeled_child("Init", node->as.var_decl.init, depth + 1);
        }
        break;
    case NODE_TRAIT_DECL:
        printf("TraitDecl: %.*s\n", node->as.trait_decl.name_length, node->as.trait_decl.name);
        print_checked_node_list(NULL, &node->as.trait_decl.methods, depth);
        break;
    case NODE_TYPE_ALIAS:
        printf("TypeAlias: %.*s", node->as.type_alias.name_length, node->as.type_alias.name);
        print_inline_type_params(node->as.type_alias.type_params,
                                 node->as.type_alias.type_param_count);
        printf("\n");
        print_visibility(depth + 1, node->as.type_alias.is_public);
        print_checked_labeled_child("Target", node->as.type_alias.target_type, depth + 1);
        break;
    case NODE_IMPL_DECL: {
        if (node->as.impl_decl.trait_name) {
            printf("ImplDecl: %.*s for %.*s", node->as.impl_decl.trait_name_length,
            ...
            printf("ImplDecl: %.*s", node->as.impl_decl.type_name_length,
                   node->as.impl_decl.type_name);
        }
        if (node->as.impl_decl.type_args.count > 0) {
            printf("<");
        ...
        printf("\n");
        print_checked_node_list(NULL, &node->as.impl_decl.methods, depth);
        break;
    }
    case NODE_USE_DECL:
        print_use_decl(node);
    ...
    case NODE_TEST_DECL:
        printf("TestDecl: \"%.*s\"\n", node->as.test_decl.name_length, node->as.test_decl.name);
        print_checked_labeled_child("Body", node->as.test_decl.body, depth + 1);
        break;

    // Statements
    case NODE_BLOCK:
        printf("Block\n");
        print_checked_node_list(NULL, &node->as.block.stmts, depth);
        break;
    case NODE_IF:
        printf("If\n");
        print_checked_labeled_child("Cond", node->as.if_stmt.cond, depth + 1);
        print_checked_labeled_child("Then", node->as.if_stmt.then_block, depth + 1);
        if (node->as.if_stmt.else_block) {
            print_checked_labeled_child("Else", node->as.if_stmt.else_block, depth + 1);
        }
        break;
    case NODE_WHILE:
        printf("While\n");
        print_checked_labeled_child("Cond", node->as.while_stmt.cond, depth + 1);
        print_checked_labeled_child("Body", node->as.while_stmt.body, depth + 1);
        break;
    case NODE_FOR:
        printf("For\n");
        print_checked_optional("Init", node->as.for_stmt.init, depth + 1);
        print_checked_optional("Cond", node->as.for_stmt.cond, depth + 1);
        print_checked_optional("Post", node->as.for_stmt.post, depth + 1);
        print_checked_labeled_child("Body", node->as.for_stmt.body, depth + 1);
        break;
    case NODE_FOREACH:
        printf("Foreach: %s", node->as.foreach_stmt.var_name);
        print_checker_flags(node);
        printf("\n");
        if (node->as.foreach_stmt.collection) {
            print_checked_labeled_child("Collection", node->as.foreach_stmt.collection, depth + 1);
        }
        print_checked_labeled_child("Body", node->as.foreach_stmt.body, depth + 1);
        break;
    case NODE_RETURN:
        printf("Return\n");
        if (node->as.return_stmt.value)
            print_ast_checked(node->as.return_stmt.value, depth + 1);
        break;
    case NODE_BREAK:
        printf("Break\n");
        break;
    ...
    case NODE_DEFER:
        printf("Defer\n");
        print_ast_checked(node->as.defer_stmt.stmt, depth + 1);
        break;
    case NODE_MATCH:
        printf("Match");
        print_checker_flags(node);
        ...
        print_indent(depth + 1);
        printf("Expr:\n");
        print_ast_checked(node->as.match_stmt.expr, depth + 2);
        print_checked_node_list("Arms", &node->as.match_stmt.arms, depth + 1);
        break;
    case NODE_MATCH_ARM:
        if (node->as.match_arm.is_wildcard) {
            printf("MatchArm: _\n");
            ...
            printf("MatchArm: <value>\n");
            print_checked_labeled_child("Pattern", node->as.match_arm.pattern_expr, depth + 1);
        } else {
            printf("MatchArm: ");
            if (node->as.match_arm.enum_name) {
                printf("%.*s::", node->as.match_arm.enum_name_length, node->as.match_arm.enum_name);
        ...
        print_checked_labeled_child("Body", node->as.match_arm.body, depth + 1);
        break;
    case NODE_EXPR_STMT:
        printf("ExprStmt\n");
        print_ast_checked(node->as.expr_stmt.expr, depth + 1);
        break;

    // Expressions
    case NODE_BINARY:
        printf("Binary: %s", token_type_name(node->as.binary.op));
        print_checker_flags(node);
        printf("\n");
        print_ast_checked(node->as.binary.left, depth + 1);
        print_ast_checked(node->as.binary.right, depth + 1);
        break;
    case NODE_UNARY:
        printf("Unary: %s\n", token_type_name(node->as.unary.op));
        print_ast_checked(node->as.unary.operand, depth + 1);
        break;
    case NODE_CALL:
        printf("Call");
        print_checker_flags(node);
        printf("\n");
        print_checked_labeled_child("Func", node->as.call.func, depth + 1);
        if (node->as.call.args.count > 0) {
            print_checked_node_list("Args", &node->as.call.args, depth + 1);
        }
        break;
    case NODE_INDEX:
        printf("Index");
        print_checker_flags(node);
        printf("\n");
        print_ast_checked(node->as.index.object, depth + 1);
        if (node->as.index.index)
            print_ast_checked(node->as.index.index, depth + 1);
        break;
    case NODE_MEMBER:
        printf("Member: %.*s", node->as.member.length, node->as.member.name);
        print_checker_flags(node);
        printf("\n");
        print_ast_checked(node->as.member.object, depth + 1);
        break;
    case NODE_ASSIGN:
        printf("Assign: %s\n", token_type_name(node->as.assign.op));
        print_ast_checked(node->as.assign.target, depth + 1);
        print_ast_checked(node->as.assign.value, depth + 1);
        break;
    case NODE_STRUCT_INIT:
        printf("StructInit\n");
        print_checked_node_list(NULL, &node->as.struct_init.fields, depth);
        break;
    case NODE_FIELD_INIT:
        printf("FieldInit: %.*s\n", node->as.field_init.name_length, node->as.field_init.name);
        print_ast_checked(node->as.field_init.value, depth + 1);
        break;
    case NODE_ENUM_VALUE:
        printf("EnumValue: %.*s::%.*s", node->as.enum_value.enum_name_length,
               node->as.enum_value.enum_name, node->as.enum_value.value_name_length,
               node->as.enum_value.value_name);
        print_checker_flags(node);
        printf("\n");
        if (node->as.enum_value.args.count > 0) {
            print_checked_node_list("Args", &node->as.enum_value.args, depth + 1);
        }
        break;
    case NODE_NEW_EXPR:
        printf("NewExpr");
        print_checker_flags(node);
        printf("\n");
        print_checked_labeled_child("Type", node->as.new_expr.type_node, depth + 1);
        print_checked_labeled_child("Init", node->as.new_expr.init, depth + 1);
        break;
    case NODE_CAST:
        printf("Cast");
        print_checker_flags(node);
        printf("\n");
        print_checked_labeled_child("Expr", node->as.cast_expr.expr, depth + 1);
        print_checked_labeled_child("TargetType", node->as.cast_expr.type_node, depth + 1);
        break;
    case NODE_TRY_EXPR:
        printf("TryExpr");
        print_checker_flags(node);
        printf("\n");
        print_ast_checked(node->as.try_expr.expr, depth + 1);
        break;
    case NODE_LAMBDA:
        printf("Lambda (%d params%s)", node->as.lambda.params.count,
               node->as.lambda.is_expr_body ? ", expr" : "");
        print_checker_flags(node);
        printf("\n");
        print_checked_node_list("Params", &node->as.lambda.params, depth);
        if (node->as.lambda.return_type) {
            print_indent(depth + 1);
            printf("ReturnType: ");
            print_ast_checked(node->as.lambda.return_type, depth + 2);
        }
        print_indent(depth + 1);
        printf("Body: ");
        print_ast_checked(node->as.lambda.body, depth + 2);
        break;
    case NODE_SLICE:
        printf("Slice");
        print_checker_flags(node);
        printf("\n");
        print_ast_checked(node->as.slice.object, depth + 1);
        if (node->as.slice.start)
            print_ast_checked(node->as.slice.start, depth + 1);
        if (node->as.slice.end)
            print_ast_checked(node->as.slice.end, depth + 1);
        break;
    case NODE_STRING_INTERP:
        printf("StringInterp (%d parts)\n", node->as.string_interp.parts.count);
    ...
    case NODE_NULL_LIT:
        ...
        break;
    case NODE_IDENT:
        printf("Ident: %.*s", node->as.ident.length, node->as.ident.name);
        print_checker_flags(node);
        printf("\n");
        break;
    ...
    case NODE_ARRAY_TYPE:
        printf("ArrayType\n");
        print_checked_labeled_child("ElemType", node->as.array_type.elem_type, depth + 1);
        if (node->as.array_type.size) {
            print_checked_labeled_child("Size", node->as.array_type.size, depth + 1);
        }
        break;
    case NODE_GENERIC_TYPE:
        printf("GenericType: %.*s\n", node->as.generic_type.base_name_length,
               node->as.generic_type.base_name);
        print_checked_node_list("TypeArgs", &node->as.generic_type.type_args, depth + 1);
        break;
    case NODE_TUPLE_TYPE:
        printf("TupleType\n");
        print_checked_node_list(NULL, &node->as.tuple_type.elem_types, depth);
        break;
    case NODE_FUNC_TYPE:
        printf("FuncType\n");
        if (node->as.func_type.param_types.count > 0) {
            print_checked_node_list("Params", &node->as.func_type.param_types, depth + 1);
        }
        print_checked_optional("ReturnType", node->as.func_type.return_type, depth + 1);
        break;
    case NODE_TUPLE_LIT:
        printf("TupleLit\n");
        print_checked_node_list(NULL, &node->as.tuple_lit.elements, depth);
        break;
