...
static Node* parse_while_stmt(Parser* parser) {
    ...
    // Check if this is a 'while (expr is Variant(...))' pattern
    if (check_token(parser, TOK_IS)) {
        advance_token(parser); // consume 'is'
        ...
        Node* node                                   = node_new(NODE_WHILE_LET, token.line, token.column);
        node->as.while_let_stmt.expr                 = expr;
        node->as.while_let_stmt.variant_name         = variant_name;
        node->as.while_let_stmt.variant_name_length  = variant_name_len;
        node->as.while_let_stmt.enum_name            = enum_name;
        node->as.while_let_stmt.enum_name_length     = enum_name_len;
        node->as.while_let_stmt.bindings             = bindings;
        node->as.while_let_stmt.binding_count        = binding_count;
        node->as.while_let_stmt.body                 = body;
        node->as.while_let_stmt.extra_cond           = extra_cond;
