...
static TokenType identifier_type(Lexer *lexer) {
    ...
    switch (start[0]) {
                ...
                switch (start[1]) {
                    case 'o':
                        if (length > 3 && start[2] == 'n') {
                            if (start[3] == 's') return check_keyword(start, length, "onst", 4, TOK_CONST);
                            if (start[3] == 't') return check_keyword(start, length, "ontinue", 7, TOK_CONTINUE);
                        }
                        break;
                }
            }
        ...
        case 'e':
            if (length > 1) {
                switch (start[1]) {
                    case 'l': return check_keyword(start, length, "lse", 3, TOK_ELSE);
                    case 'n': return check_keyword(start, length, "num", 3, TOK_ENUM);
                }
            }
            break;
        case 'f':
            if (length > 1) {
                switch (start[1]) {
                    case 'a': return check_keyword(start, length, "alse", 4, TOK_FALSE);
                    case 'o': return check_keyword(start, length, "or", 2, TOK_FOR);
                    case 'u': return check_keyword(start, length, "unc", 3, TOK_FUNC);
                }
            }
            break;
        case 'i':
...
Token lexer_next(Lexer *lexer) {
    ...
    switch (c) {
        ...
        case '&':
            if (match(lexer, '&')) return make_token(lexer, TOK_AMP_AMP);
            if (match(lexer, '=')) return make_token(lexer, TOK_AMP_EQ);
            return make_token(lexer, TOK_AMP);
        case '|':
            if (match(lexer, '|')) return make_token(lexer, TOK_PIPE_PIPE);
            if (match(lexer, '=')) return make_token(lexer, TOK_PIPE_EQ);
        ...
        case '<':
            if (match(lexer, '<')) return make_token(lexer, TOK_LT_LT);
            if (match(lexer, '=')) return make_token(lexer, TOK_LT_EQ);
            return make_token(lexer, TOK_LT);
        case '>':
            if (match(lexer, '>')) return make_token(lexer, TOK_GT_GT);
            if (match(lexer, '=')) return make_token(lexer, TOK_GT_EQ);
